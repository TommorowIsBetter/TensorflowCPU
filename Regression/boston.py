...
@author:Wang Yan
@ide:PyCharm
@time:2019/7/30 21:48
"""
# boston房价问题通过卷积神经网络来实现完成
import numpy as np
...
# 取x的第三列，第四列，第五列数据
x_3 = x[:, 3:6]
# 随意给x增加了3列，x变为16列，可以reshape为4*4矩阵了，目标是可以做CNN的输入数据
...
for i in range(500):
    ...
    print(i, '误差=', sess.run(loss, feed_dict={xs: train_x_disorder, ys: train_y_disorder, keep_prob: 1.0}))  # 输出loss值
# 可视化
...
line1, = axes.plot(range(len(prediction_value)), prediction_value, 'b--', label='predict value', linewidth=2)
line3, = axes.plot(range(len(test_y_disorder)), test_y_disorder, 'g', label='real value')
axes.grid()
fig.tight_layout()
